...
//         backgroundColor: '#f1f5f9', // Light background matching Dashboard
//         color: '#1e293b', // Dark text
//         fontFamily: 'system-ui, -apple-system, sans-serif',
//         backgroundImage: "radial-gradient(#cbd5e1 1px, transparent 1px)", // Dot pattern
...
//             style={{
//               padding: '10px 24px',
//               backgroundColor: canSubmit ? '#ecfdf5' : '#f1f5f9', // Green-50 / Slate-100
...
//                               backgroundColor: msg.role === 'user' ? '#e0f2fe' : '#f1f5f9', // Sky-100 vs Slate-100
//                               color: '#0f172a',
...
//                           {partId ? `Looking for ${partId}.pdf` : 'No part ID specified'}
//                         </p>
//                       </div>
//                     </div>
//                   )}
...
//               display: 'flex',
//               flexDirection: 'column',
//               backgroundColor: '#1e293b', // Keep output dark for contrast
//               borderRadius: '16px',
            ...
            if (data.status === "accepted") {
                ...
                try {
                    ...
                    if (storedUser) {
                        ...
                        if (user.id && partId) {
                            console.log("📤 Saving coding completion:", { user_id: user.id, part_id: partId })

                            const saveResponse = await fetch("http://localhost:8000/submit/coding", {
                                method: "POST",
                                headers: { "Content-Type": "application/json" },
                                body: JSON.stringify({
                                    user_id: user.id,
                            ...
                            if (saveResponse.ok) {
                                console.log("✅ Coding completion saved successfully!")
                            } else {
                                console.warn("⚠️  Failed to save coding completion")
                            }
                        } else {
                            console.warn("⚠️  Missing user.id or partId:", { userId: user.id, partId })
                        }
                    } else {
                        console.warn("⚠️  No user found in localStorage")
                    }
                } catch (saveErr) {
                    console.error("❌ Error saving completion:", saveErr)
                }
            } else {
                setCanSubmit(false)
            }
